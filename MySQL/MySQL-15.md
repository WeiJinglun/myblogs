...
## 日志相关问题
 ...
 binlog（归档日志）和 redo log（重做日志）的两阶段提交的图：

![MySQL-15-1](img/MySQL-15-1.jpeg)

这个例子里面，没有显式地开启事务，因此这个 update 语句自己就是一个事务，在执行完成后提交事务时，就会用到这个“commit 步骤“。

分析一下**在两阶段提交的不同时刻，MySQL 异常重启会出现什么现象。**
...
如果在时刻 B，也就是 binlog 写完，redo log 还没 commit 前发生 crash，那崩溃恢复的时候 MySQL 会怎么处理？
...
![MySQL-15-2](img/MySQL-15-2.jpeg)
...
| session A（操作逻辑：A 喜欢 B）                              | session B（操作逻辑：B 喜欢 A）                              |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| begin;<br />select * from \`like\` where user_id = B and liker_id = A;(返回空) |                                                              |
|                                                              | begin;<br />select * from \`like\` where user_id = A and liker_id = B;(返回空) |
...
/* 这里 update 操作，如果原来存在该行，relation_ship = 2，
	那么 2|1 =3，如果 relation_ship = 3，3|1 = 3，很巧妙的或操作
*/
...
/* 这里 update 操作，如果原来存在该行，relation_ship = 1，
	那么 1|2 =3，如果 relation_ship = 3，3|2 = 3，很巧妙的或操作
*/
...
| session A                                                    | session B                    |
| ------------------------------------------------------------ | ---------------------------- |
| begin;<br />select * from t where id =1;(返回（1，2）)       |                              |
|                                                              | update t set a=3 where id=1; |
| update t set a=3 where id=1;<br /><br />Query OK，0 rows affected (0.00 sec)<br />Rows matched: 1 Changed: 0 Warning: 0 |                              |
