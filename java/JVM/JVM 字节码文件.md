...
![img](img/JVM字节码文件-8.jpg)

（5）**ladd**

把栈顶两个 long 型数值出栈后相加，并将结果入栈。

![img](img/JVM字节码文件-9.jpg)

（6）**getstatic #3**

根据偏移获取静态属性的值，并把这个值 push 到操作数栈上。

![img](img/JVM字节码文件-10.jpg)

（7）**ladd**

再次执行 ladd。

![img](img/JVM字节码文件-11.jpg)
...
![img](img/JVM字节码文件-12.jpg)

（9）**lload_3**

正好与上面相反。上面是变量存入，我们现在要做的，就是把这个变量 ret，压入虚拟机栈中。

![img](img/JVM字节码文件-13.jpg)
